...
const SupportTrips = () => {
    ...
    const saveHandler = () => {
        ...
        if (isNew) {
            ...
            axiosAuth.post('/support/travel/create', queryData)
                .then(({data})=> {
                    setList([ data, ...list ])
            ...
            axiosAuth.put('/support/travel/update', queryData)
                .then(()=> {
                    const newList = list.map( obj =>
                        obj.id===editData.id ? { ...editData, is_video: switchPos==='video' } : obj )
                    setList(newList)
                    toast.success('Данные сохранены')
                })
                .catch(()=>toast.error('Произошла ошибка'))
    ...
    const deleteHandler = id => {
        ...
        axiosAuth.delete('/support/travel/delete', { data: { id } })
            .then(()=> {
    ...
    const deleteTickerHandler = id => {
        ...
        if (isLink) {
            axiosAuth.delete('/support/travel/image/delete', { data: { id } })
                .then(()=> {
    ...
    const saveNewPosition = e => {
        ...
        axiosAuth.post('/support/travel/position', { id, new_position })
        ...
        <ul className={ s.list }>
            ...
            >
                <SortableContext items={list} strategy={verticalListSortingStrategy}>
                    {
                        list.map( (item, i) =>
                            <TripsItem key={item.id} {...{item, i, editHandler, deleteHandler}}/>
                        )
                    }
                </SortableContext>
            </DndContext>
            ...
            />
            {
                switchPos==='video'
                    ...
                    <>
                        <FileInput
                            label='Загрузка фото'
                            setValue={ image => setEditData({...editData, images: [ {id: nanoid(), image}, ...editData.images ]}) }
                        />
                        <DndContext

...
const TeachersGallery = () => {
    ...
    useEffect(() => {
        axiosAuth('/teacher/gallery')
            .then(({ data }) => {
                ...
                setPhotoList(data.items[0].items)
    ...
    const deleteHandler = (id, type) => {
        if (type === 'category') {
            ...
            axiosAuth
                ...
                .then(() => {
                    const newList = photoList.filter((item) => item.id !== id)
    ...
    const saveNewPosition = (e, type) => {
        ...
        const currentLink = `/teacher/gallery/${type==='category' ? 'group' : 'item'}/position`
        axiosAuth
            .post(currentLink, { id, new_position })
    ...
    return (
        <>
            ...
            <EditAside state={ isOpenAsidePhoto } setState={ setIsOpenAsidePhoto } title={ dataType === 'photo' ? 'Добавить фото' : 'Добавить видео' }>
                <FileInput
                    label={ dataType === 'photo' ? 'Загрузка изображения' : 'Загрузка превью' }
